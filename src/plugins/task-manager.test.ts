...
describe('task manager', () => {
  ...
  it('should wait for __cleanup__ queue to complete before processing other queues', async () => {
    const logs: string[] = [];
    const tm = new TaskManager();

    // Add cleanup task that takes some time
    tm.addTask('__cleanup__', async () => {
      logs.push('cleanup started');
      await delay(100);
      logs.push('cleanup ended');
    });

    // Add spec tasks immediately after - they should wait for cleanup
    tm.addTask('spec1', async () => {
      logs.push('spec1 started');
      await delay(10);
      logs.push('spec1 ended');
    });

    tm.addTask('spec2', async () => {
      logs.push('spec2 started');
      await delay(10);
      logs.push('spec2 ended');
    });

    await tm.flushAllTasks();

    // Cleanup must complete before any spec tasks start
    expect(logs).toEqual([
      'cleanup started',
      'cleanup ended',
      'spec1 started',
  ...
  it('should not wait for cleanup if no __cleanup__ queue exists', async () => {
    const logs: string[] = [];
    const tm = new TaskManager();

    // Add spec tasks without any cleanup - they should start immediately
    tm.addTask('spec1', async () => {
      logs.push('spec1 started');
      await delay(50);
      logs.push('spec1 ended');
    ...
    // Both specs should run in parallel (spec2 ends before spec1 due to shorter delay)
    expect(logs).toEqual(['spec1 started', 'spec2 started', 'spec2 ended', 'spec1 ended']);
  });

  it('should allow spec tasks to start after cleanup completes even if added later', async () => {
    const logs: string[] = [];
    const tm = new TaskManager();
    ...
    // Now add spec tasks after cleanup is done - they should not wait
    tm.addTask('spec1', async () => {
      logs.push('spec1 started');
      logs.push('spec1 ended');
    });
    ...
    expect(logs).toEqual(['cleanup started', 'cleanup ended', 'spec1 started', 'spec1 ended']);
  });
});
