...
describe('video when test pass', () => {
  let results: PreparedResults;
  outputDebugGenerate(__dirname);

  beforeAll(async () => {
    results = await prepareResults(__dirname, {
      env: { video: false, allureAddVideoOnPass: 'false' },
    });
  });

  it('should not have video for all tests in suite in after hook when tests passed and no video setting', async () => {
    ...
    expect(suite?.name).toEqual('video suite (allureAddVideoOnPass false)');
    ...
    expect(excludeCovAfters).toEqual([]);
  });
});
