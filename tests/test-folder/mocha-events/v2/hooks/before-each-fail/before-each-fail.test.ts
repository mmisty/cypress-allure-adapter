...
describe('before each hook fail', () => {
  ...
  describe('test 1', () => {
    ...
    it('should have after each hook step', () => {
      const afterEachHook = test?.steps.find(
        s => s.name === '"after each" hook',
      );
      expect(afterEachHook?.status).toEqual('passed');
    });
