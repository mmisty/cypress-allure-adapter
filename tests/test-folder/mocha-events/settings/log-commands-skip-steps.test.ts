...
describe('should skip hooks steps', () => {
...
`,
    ...
    {
      allureSkipSteps:
        '"before each" hook,"after each" hook,"before all" hook,"after all" hook,*generateReport,*mergeUnitTestCoverage,*collectBackendCoverage',
    },
  );
  describe('check results', () => {
    ...
    it('should check before all/after all hooks', () => {
      ...
      expect({ befores, afters }).toEqual({
        afters: [
          {
            name: '"after all" hook: NAMED HOOK',
            steps: [
              {
                name: 'log: after',
                status: 'passed',
